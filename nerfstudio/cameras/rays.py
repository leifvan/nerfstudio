...
@dataclass
class PiecewiseRayBundle(TensorDataclass):
    """A bundle of ray parameters."""

    # TODO(ethan): make sure the sizes with ... are correct
    origins: Float[Tensor, "*batch 3"]
    """Ray origins (XYZ)"""
    directions: Float[Tensor, "*batch n_segments 3"]
    """Unit ray direction vector"""
    start_t: Float[Tensor, "*batch n_segments 1"]
    """Ray position where segment starts"""
    pixel_area: Float[Tensor, "*batch 1"]
    """Projected area of pixel a distance 1 away from origin"""
    camera_indices: Optional[Int[Tensor, "*batch 1"]] = None
    """Camera indices"""
    ...
    def __len__(self) -> int:
        num_rays = torch.numel(self.origins) // self.origins.shape[-1] // self.origins.shape[-2]
        return num_rays

    def sample(self, num_rays: int) -> "PiecewiseRayBundle":
        """Returns a RayBundle as a subset of rays.
    ...
    # def get_row_major_sliced_ray_bundle(self, start_idx: int, end_idx: int) -> "PiecewiseRayBundle":
    #     """Flattens RayBundle and extracts chunk given start and end indices.
    ...
    def get_ray_samples(
        ...
        Returns:
            Samples projected along ray.
        """
        # TODO handle intersection locations cleanly
